...
class DashboardItem {
    ...
    createMarkUp() {
        ...
        const todo = title.toLowerCase().replace(/ /g, "-");
        const {current, previous} = timeframes[this.view.toLowerCase()];

        this.container.insertAdjacentHTML('beforeend',
            `
                <div class="dashboard__item dashboard__item--${todo}">
          <article class="tracking-card">
            <header class="tracking-card__header">
              <h4 class="tracking-card__title">${title}</h4>
              <img class="tracking-card__icon-menu" src="images/icon-ellipsis.svg" alt="menu" >
            </header>
        ...
        this.time = this.container.querySelector(`.dashboard__item--${todo} .tracking-card__time`);
        this.prev = this.container.querySelector(`.dashboard__item--${todo} .tracking-card__prev-period`);
    }
